...
function mapPackageData(item) {
  ...
  if (city?.pricingByRoomType && Array.isArray(city.pricingByRoomType)) {
    ...
    if (firstPricing?.priceDoubleOcc) {
      priceDoubleOcc = formatPriceWithSymbol(firstPricing.priceDoubleOcc);
    }
  }
  ...
  const itinerariesHoverText = itinerariesArr.slice(0, 5).map(it => `Day ${it.day} : ${decodeHtml(it.title)}`).join('\n');
  ...
  return {
    ...
    title: decodeHtml(pkg?.Name || "Package"),
    isWishlisted: false,
    ...
    pricing: {
      oldPrice: null,
      price: priceDoubleOcc || (item?.minPrice ? formatPriceWithSymbol(item.minPrice) : null),
...
export default function PackagesCarousel({
  ...
  const sizes = useMemo(
    () => ({
      cardW: "w-[calc(95%)] sm:w-[calc(50%+40px)] md:w-[calc(45%+40px)] lg:w-[calc(32%+40px)] xl:w-[calc(28%+40px)] 2xl:w-[calc(26%+40px)]", // mobile shows 95% current + 5% next peek
